    ...
    for i, (f, entry) in enumerate(zip(valid_files, pending)):
        try:
            if upload_type == "📄 New RFP":
                ingest_new_rfps(file_paths=[f.name for f in valid_files], session_id=session_id)
            else:
                ingest_session_documents(file_paths=[f.name for f in valid_files], session_id=session_id)

            # Make the session ID available to the agent tools immediately
...
def remove_chip_handler(chip_id: str, chips_data: list, session_id: str):
    ...
    if removed:
        try:
            if removed["type"] == "📄 New RFP":
                delete_rfp_collection(session_id=removed["session_id"])
            else:
                delete_doc_collection(session_id=removed["session_id"])
        except Exception as exc:
            print(f"[Remove chip] ChromaDB deletion error: {exc}")
    # ─────────────────────────────────────────────────────────────────────────
